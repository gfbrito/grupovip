...
export default function LaunchMessagesPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        scheduledAt: '',
        delayMin: 5,
        delayMax: 15, // Segundos
        applyToAll: true,
    });

    // Test Modal State
    const [testModalOpen, setTestModalOpen] = useState(false);
    const [testMessageId, setTestMessageId] = useState<number | null>(null);
    ...
    const openLogsModal = async (msgId: number) => {
        ...
        setLogsLoading(true);
        try {
            const res = await api.get(`/launches/${id}/messages/${msgId}/logs`);
    ...
    const handleCreate = async (e: React.FormEvent, sendNow: boolean = false) => {
        ...
        try {
            ...
            // Validar data apenas se não for enviar agora
            if (!sendNow) {
            ...
            // Confirmação para enviar agora
            if (sendNow && !confirm('Enviar esta mensagem imediatamente para todos os grupos?')) {
                return;
            }

            await api.post(`/launches/${id}/messages`, {
                ...formData,
                delayMin: formData.delayMin * 1000, // Converter para ms
                delayMax: formData.delayMax * 1000, // Converter para ms
                sendNow,
            });
    ...
    const handleDelete = async (messageId: number) => {
        ...
        try {
            await api.delete(`/launches/${id}/messages/${messageId}`);
    ...
    const handleSendNow = async (messageId: number) => {
        ...
        try {
            await api.post(`/launches/${id}/messages/${messageId}/send`);
    ...
    const openTestModal = (msgId: number) => {
        setTestMessageId(msgId);
        setTestModalOpen(true);
        // Selecionar primeiro grupo por padrão se houver
        if (groups.length > 0) setTestGroupId(groups[0].id.toString());
    };

    const handleSendTest = async () => {
        if (!testMessageId || !testGroupId) return;

        try {
            await api.post(`/launches/${id}/messages/${testMessageId}/test`, {
                groupId: parseInt(testGroupId)
            });
            addToast({ type: 'success', title: 'Sucesso', message: 'Mensagem de teste enviada.' });
            setTestModalOpen(false);
        } catch (error: any) {
            addToast({ type: 'error', title: 'Erro', message: error.response?.data?.error || 'Erro ao enviar teste.' });
        }
    };
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="mb-6 flex justify-between items-center">
                <div>
                    <h2 className="text-xl font-bold text-slate-900">Mensagens Agendadas</h2>
                    <p className="text-sm text-slate-500">
                        Programe envios de texto, mídia e áudio para seus grupos.
                    </p>
                </div>
            ...
            {showForm && (
                <Card className="mb-8 border-indigo-100 shadow-md">
                    <Card.Header className="bg-indigo-50/50">
                        <h3 className="font-semibold text-indigo-900">Programar Nova Mensagem</h3>
                    </Card.Header>
                    <Card.Body>
                        <form onSubmit={handleCreate} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <Input
                                        placeholder="Ex: Aviso de Live Iniciada"
                                ...
                                {formData.type !== 'TEXT' && (
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1">
                                            Arquivo da Mídia {formData.type === 'VIDEO' ? '(Até 30MB)' : '(Até 5MB)'}
                                        </label>
                                        <input
                                            type="file"
                                            ...
                                            accept={
                                                ...
                                                formData.type === 'VIDEO' ? 'video/mp4,video/mpeg' :
                                                formData.type === 'AUDIO' ? 'audio/mpeg,audio/mp4,audio/ogg' :
                                                '.pdf,.doc,.docx'
                                            }
                                            onChange={async (e) => {
                                                ...
                                                try {
                                                    ...
                                                    addToast({ type: 'success', title: 'Upload concluído', message: 'Mídia salva no servidor.' });
                                                } catch (err: any) {
                                                    addToast({ type: 'error', title: 'Erro', message: err.response?.data?.error || 'Falha no upload.' });
                                                    e.target.value = '';
                                                }
                                            }}
                                        ...
                                        />
                                        {formData.mediaUrl && (
                                            <p className="text-xs text-green-600 mt-2 font-medium">✓ Arquivo carregado e pronto para envio.</p>
                                        )}
                                    </div>
                                )}
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 mt-1">
                                        Variáveis: {'{nome}'} (nome do lead/grupo), {'{nome_lancamento}'}, {'{n}'}
                                    </p>
                                </div>
                                ...
                                <div className="flex items-center gap-2 pt-2">
                                    ...
                                    />
                                    <label htmlFor="applyAllMsg" className="text-sm text-slate-700">Enviar para todos os grupos</label>
                                </div>
                            ...
                            <div className="bg-slate-100 p-6 rounded-xl border border-slate-200">
                                ...
                                <div className="bg-[#e5ddd5] p-4 rounded-lg min-h-[400px] flex flex-col relative overflow-hidden">
                                    ...
                                    <div className="flex-1"></div> {/* Spacer to push msg down */}
                                ...
                                <div className="flex-1 p-4 flex flex-col justify-between">
                                    <div>
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <div className="flex items-center gap-1 ml-4 self-start">
                                                <Button variant="ghost" size="sm" title="Enviar Teste" onClick={() => openTestModal(msg.id)}>
                                    ...
                                    <div className="flex items-center justify-between text-xs text-slate-500 mt-2">
                                        ...
                                        <span className="flex items-center gap-1">
                                            {msg.applyToAll ? 'Todos os grupos' : 'Grupos selecionados'}
                                            <button
            ...
            {testModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
                        <h3 className="text-lg font-bold text-slate-900 mb-4">Enviar Mensagem de Teste</h3>
                        <p className="text-sm text-slate-600 mb-4">
                            Selecione um grupo para enviar a mensagem de teste imediatamente.
                        </p>

                        <div className="mb-6">
                            <label className="block text-sm font-medium text-slate-700 mb-1">Grupo de Destino</label>
                            <select
                            ...
                            >
                                {groups.map(g => (
                                    <option key={g.id} value={g.id}>{g.name} ({g.remoteJid})</option>
                                ))}
                            </select>
                        </div>

                        <div className="flex justify-end gap-2">
                            <Button variant="ghost" onClick={() => setTestModalOpen(false)}>Cancelar</Button>
                            <Button onClick={handleSendTest}>Enviar Teste</Button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[80vh] overflow-hidden flex flex-col">
                        ...
                        <div className="px-6 py-4 border-b border-slate-100">
                            <h3 className="text-lg font-bold text-slate-900">Status de Envio por Grupo</h3>
                            <p className="text-sm text-slate-500 mt-1">
                                {messageLogs.length} grupo{messageLogs.length !== 1 ? 's' : ''} processado{messageLogs.length !== 1 ? 's' : ''}
                            </p>
                        </div>
                                    ...
                                    {messageLogs.map((log, idx) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="font-medium text-slate-800 truncate" title={log.groupName}>
                                                    {log.groupName}
                                                </p>
                                                {log.error && (
